'use strict';

exports.__esModule = true;
exports.default = {
  el: {
    colorpicker: {
      confirm: 'ठीक है',
      clear: 'साफ़'
    },
    datepicker: {
      now: 'अब',
      today: 'आज',
      cancel: 'रद्द करें',
      clear: 'साफ़',
      confirm: 'ठीक है',
      selectDate: 'तारीख चुनें',
      selectTime: 'समय चुनें',
      startDate: 'प्रारंभ तिथि',
      startTime: 'प्रारंभ समय',
      endDate: 'समाप्ति तिथि',
      endTime: 'समाप्ति समय',
      prevYear: 'पिछला वर्ष',
      nextYear: 'अगला वर्ष',
      prevMonth: 'पिछला महीना',
      nextMonth: 'अगला महीना',
      year: '',
      month1: 'जनवरी',
      month2: 'फ़रवरी',
      month3: 'मार्च',
      month4: 'अप्रैल',
      month5: 'मई',
      month6: 'जून',
      month7: 'जुलाई',
      month8: 'अगस्त',
      month9: 'सितंबर',
      month10: 'अक्टूबर',
      month11: 'नवंबर',
      month12: 'दिसंबर',
      week: 'सप्ताह',
      weeks: {
        sun: 'रविवार',
        mon: 'सोमवार',
        tue: 'मंगलवार',
        wed: 'बुधवार',
        thu: 'गुरुवार',
        fri: 'शुक्रवार',
        sat: 'शनिवार'
      },
      months: {
        jan: 'जनवरी',
        feb: 'फ़रवरी',
        mar: 'मार्च',
        apr: 'अप्रैल',
        may: 'मई',
        jun: 'जून',
        jul: 'जुलाई',
        aug: 'अगस्त',
        sep: 'सितंबर',
        oct: 'अक्टूबर',
        nov: 'नवंबर',
        dec: 'दिसंबर'
      }
    },
    select: {
      loading: 'लोड हो रहा है',
      noMatch: 'कोई मिलती जुलती डेटा नहीं',
      noData: 'कोई डेटा नहीं',
      placeholder: 'चयन करें'
    },
    cascader: {
      noMatch: 'कोई मिलती जुलती डेटा नहीं',
      loading: 'लोड हो रहा है',
      placeholder: 'चयन करें',
      noData: 'कोई डेटा नहीं'
    },
    pagination: {
      goto: 'जाएँ',
      pagesize: '/पेज',
      total: 'कुल {total}',
      pageClassifier: ''
    },
    messagebox: {
      title: 'संदेश',
      confirm: 'ठीक है',
      cancel: 'रद्द करें',
      error: 'अवैध इनपुट'
    },
    upload: {
      deleteTip: 'हटाने के लिए डिलीट दबाएं',
      delete: 'हटाएं',
      preview: 'पूर्वावलोकन',
      continue: 'जारी रखें'
    },
    table: {
      emptyText: 'कोई डेटा नहीं',
      confirmFilter: 'पुष्टि करें',
      resetFilter: 'रीसेट',
      clearFilter: 'सब',
      sumText: 'योग'
    },
    tree: {
      emptyText: 'कोई डेटा नहीं'
    },
    transfer: {
      noMatch: 'कोई मिलती जुलती डेटा नहीं',
      noData: 'कोई डेटा नहीं',
      titles: ['सूची 1', 'सूची 2'], // to be translated
      filterPlaceholder: 'कीवर्ड दर्ज करें', // to be translated
      noCheckedFormat: '{total} आइटम', // to be translated
      hasCheckedFormat: '{checked}/{total} चेक किए गए' // to be translated
    },
    image: {
      error: 'असफल'
    },
    pageHeader: {
      title: 'पीछे जाएं' // to be translated
    },
    popconfirm: {
      confirmButtonText: 'हाँ',
      cancelButtonText: 'नहीं'
    },
    empty: {
      description: 'कोई डेटा नहीं'
    }
  }
};
